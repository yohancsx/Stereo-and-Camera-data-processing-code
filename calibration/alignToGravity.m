...
% Path to the input 3D points spreadsheet (DLTdv8 xyzpts format) - the file
% main_calibration_reconstruction.m writes at its step 5.
% Leave "" to be prompted.
%CHANGE THIS
inputPath = "";
...
if strlength(inputPath) == 0
    [fIn, pIn] = uigetfile({'*.csv','Reconstructed 3D points (*.csv)'}, ...
        'Select the xyzpts CSV to align');
    if isequal(fIn, 0), error('No file selected - stopping.'); end
    inputPath = string(fullfile(pIn, fIn));
end
assert(isfile(inputPath), 'File not found: %s', inputPath);
