...
if isempty(cfg.maskFiles)
    ...
    if isempty(cfg.calibFile)
        if isempty(p)
            warning('stereoPairReconstruct:noCalib', ...
                ['No calibration .mat found in %s - proceeding WITHOUT ' ...
                 'undistortion.'], cfg.dataDir);
        else
            cfg.calibFile = fullfile(cfg.dataDir, p(1).name);
        end
    end
end
...
M = vh.loadMasks(cfg.maskFiles, cfg.calibFile, ...
    ...
    'verbose', cfg.verbose);
assert(isequal(sort(cfg.camOrder(:).'), 1:nCams), 'stereoPairReconstruct:camOrder', ...
    ['cfg.camOrder must be a permutation of 1:%d for this %d-camera dataset ' ...
     '(the default [4 1 3 2] is a worked example for a different, 4-camera ' ...
     'rig) - set cfg.camOrder yourself, e.g. 1:%d if the DLT columns and ' ...
     'mask files are already in the same order.'], nCams, nCams, nCams);
